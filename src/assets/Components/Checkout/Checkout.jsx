import { serverTimestamp, addDoc, doc, collection, getDoc, updateDoc } from 'firebase/firestore';
 ...
 */                setOrderId(res.id)
                ...
                .catch((error) => {
                    ...
                    alert('Hubo un error al procesar el pedido. Por favor, inténtalo de nuevo más tarde.');
                });
            }
    };
        ...
        <div >
            {orderId !== '' ?
                <div className='thanksContainer'>
                    ...
                    <h5>Tu numero de pedido es el {orderId}, pronto te enviaremos un correo con todos los datos de tu compra.</h5>
                ...
                <div className='checkoutContainer' >
                    <form className='dataUsersContainer' >
                        ...
                        <label className='dataUsers__name'>
                            <input name='name' placeholder="Nombre" type="text" value={user.name || ''} onChange={userData} />
                            <input name='lastname' placeholder="Apellidos" type="text" value={user.lastname || ''} onChange={userData} />
                        </label>
                        <label className='dataUsers__email'>
                            <input name='email' placeholder="Email" type="email" value={user.email || ''} onChange={userData} />
                            <input name='second-email' placeholder="Ingresa de nuevo tu email" type="email" value={validateEmail} onChange={(e) => setValidateEmail(e.target.value)} />
                            <input name='phone' placeholder="Telefono" type="text" value={user.phone || ''} onChange={userData} />
                            <h4>Informacion adicional (Opcional)</h4>
                            <textarea rows="4" cols="50" name='note' placeholder="Notas del pedido" value={user.note || ''} onChange={userData} />
                        </label>
                    ...
                    <div className='ordersContainer'>
                        <div className='orderDetails'>
                            ...
                            <div className='orderDetails__totalContainer'>
                                <h3>Total a pagar: <span className='orderDetails__span'>{totalBuy()}$</span></h3>
                                <Link to='/cart'><Button1>Volver a Carrito</Button1></Link>
                                <Button1 font='20px' onClick={checkout}>Pagar</Button1>
                            </div>
                        </div>
                        <div className='orderPayout'>
                        </div>
                    </div>
                </div>
            }
